...
class AWSCostReporter:
    ...
    def format_message(self, dates, costs):
        service_details = "\n".join(
            [
                f"🔹 {service} – **${cost:.2f}**"
                for service, cost in costs["service_costs"]
            ]
        )
    ...
    def send_to_webex(self, message):
        try:
            ...
            response = self.http.request(
                "POST", self.webhook_url, body=encoded_msg, headers=headers
            )

            if response.status == 204:
                return {
                    "statusCode": 204,
                    "body": "Message sent successfully to Webex Chat!",
                }
            return {
                "statusCode": response.status,
                "body": f"Failed to send message to Webex Chat: {response.data}",
            }
...
def lambda_handler(event, context):
    try:
        ...
        service_costs, yesterday_total = reporter.get_service_costs(
            dates["start_date"], dates["end_date"]
        )
        forecasted = reporter.get_forecasted_spend(
            dates["three_days_ago"], dates["end_date"]
        )
        month_total = reporter.get_month_spend(
            dates["first_day_of_month"], dates["end_date"]
        )
